...
onValue(webInDB, function(snapshot) {
    if (snapshot.exists()) {
        ...
        for (let i = 0; i < itemsArray.length; i++) {
            ...
            <iframe sandbox="" class="web-link-popup" id="web-link${i}" src="https://${currentLink}"></iframe>
            </a>
            </li>`
        }
...
onValue(webInDB2, function(snapshot) {
    if (snapshot.exists()) {
        ...
        for (let i = 0; i < itemsArray.length; i++) {
            ...
            <a class="webLink" href="https://${currentLink}" target="_blank">${currentName}
            <iframe sandbox="" class="web-link-popup" id="web-link${i+databaseLengthFirst}" src="https://${currentLink}"></iframe>
            </a>
            </li>`
        }    
    } else {
